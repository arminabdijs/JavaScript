...
function getRandomImage() {
  let index = Math.floor(Math.random() * arrSrcImage.length);

  return arrSrcImage[index];
}
...
// عکس قبلی را سریع نشان بده
const oldImage = localStorage.getItem('background');

if (oldImage) {
  setBackground(oldImage);
}
...
img.onload = () => {
  ...
  setTimeout(() => {
    ...
    localStorage.setItem('background', newImage);
